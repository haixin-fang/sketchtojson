...
const styleParser = function (style, attributedString, layer) {
  ...
  if (style.textStyle && attributedString && attributedString.archivedAttributedString) {
    ...
    if (style.textStyle.encodedAttributes) {
      ...
      if (encodedAttributes.NSColor) decodedNSColor = parseArchive(encodedAttributes.NSColor._archive);
      if (encodedAttributes.NSParagraphStyle) decodedNSParagraphStyle = parseArchive(encodedAttributes.NSParagraphStyle._archive);
      if (encodedAttributes.MSAttributedStringFontAttribute) decodedMSAttributedStringFontAttribute = parseArchive(encodedAttributes.MSAttributedStringFontAttribute._archive);
    }
...
function parseArchive(base64String) {
  const buf2 = window.Buffer.from(base64String, "base64");
  const obj = parseBuffer(buf2);
